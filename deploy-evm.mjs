...
// CONFIG
// ============================================
const RPC_URL = process.env.VITE_QF_RPC_URL;
if (!RPC_URL) throw new Error('VITE_QF_RPC_URL environment variable is required');
const CHAIN_ID = 42;
...
// RESERVED NAMES LIST
// ============================================
const reservedJson = JSON.parse(readFileSync('reserved-names.json', 'utf-8'));
const reservedNames = [...new Set(reservedJson.names)];
console.log(`Loaded ${reservedNames.length} reserved names from reserved-names.json`);
...
# QF Network Substrate RPC Endpoint (use ws:// for localhost, wss:// for production)
VITE_QF_RPC_URL=${process.env.VITE_QF_RPC_URL || 'ws://localhost:9944'}
`;
